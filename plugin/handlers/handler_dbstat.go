...
func DbStatHandler(ctx context.Context, conn PostgresClient, _ string,
  ...
  SELECT json_object_agg(coalesce (datname,'null'), row_to_json(T))
	...
	return runDbStatQuery(ctx, conn, query)
}

// DbStatSumHandler executes select from pg_catalog.pg_stat_database
// command for each database and returns JSON if all is OK or nil otherwise.
func DbStatSumHandler(ctx context.Context, conn PostgresClient, _ string,
	_ map[string]string, _ ...string) (interface{}, error) {
	var query string
...
// runDbStatQuery runs the query constructed by the handler and returns JSON if all is OK or nil otherwise
func runDbStatQuery(ctx context.Context, conn PostgresClient, query string) (interface{}, error) {
	var statJSON string
